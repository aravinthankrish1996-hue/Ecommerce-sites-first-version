                    ...
                    <div class="row no-gutters trending-product-grid">
                        ...
                        <div class="col-10">
                            <div class="tab-content tp-tab-content" id="trendingTabContent">
                                <div v-for="(catItem, index) in getShortArray(10)" :key="catItem.id"
                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-6">
                            <div class="new-arrival-nav mb-35">
                                <button v-for="(catItem, index) in getShortArray(10)" :key="catItem.id"
                                    :class="showActiveClass(1, index)" :data-filter="'.cat-' + catItem.id">
                                    {{ catItem.name }}
                                </button>
                            </div>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="shoes-banner-content">
                                        <h6>Product Items</h6>
                                        <h2>9 Best <span>Product for</span> Standing All Day Review 2025</h2>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="shoes-banner-bg" data-background="/frontend_assets/img/bg/shoes-banner_bg.jpg">
                            <div class="row">
                                <div class="col-12">
                                    <div class="shoes-banner-content">
                                        <h6>Product Items</h6>
                                        <h2>3 Best <span>Product for</span> Standing All Day Review 2025</h2>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="shoes-banner-bg" data-background="/frontend_assets/img/bg/shoes-banner_bg.jpg">
                            <div class="row">
                                <div class="col-12">
                                    <div class="shoes-banner-content">
                                        <h6>Product Items</h6>
                                        <h2>8 Best <span>Product for</span> Standing All Day Review 2025</h2>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-for="item in homeBrands" :key="item.id" class="col">
                            <div class="insta-post-item">
                                <a href="#">
                                    ...
                                    <img :src="item.image_url" style="height:385px;width: 350px;" alt="">
                                </a>
                            </div>
                        </div>
                    </div>
    ...
    methods: {
        ...
        handleAddToCart(item, addToCartFunction) {
            try {
                ...
                const productAttrId = item.product_attr_id || productAttributes[0].id;
                const qty = 1; // Default quantity

                // Call the addToCart function from Layout
                if (addToCartFunction && typeof addToCartFunction === 'function') {
                    addToCartFunction(productId, productAttrId, qty);
                } else {
        ...
        async getHomeBanner() {
            try {
                const response = await axios.get(getUrlList().getHomeData);
                ...
                if (response.status === 200 && response.data.data) {
                    ...
                    console.log('banner loaded successfully:', this.homeBanner);
                    console.log('category loaded successfully:', this.homeCategories);
                    console.log('brand loaded successfully:', this.homeBrands);
                    console.log('product loaded successfully:', this.homeProducts);
                } else {
                    console.log('Data not found or API returned an empty response.');
                }
            } catch (error) {
                console.error('There was an error fetching the banner:', error.response || error);
            }
        }
    }
}

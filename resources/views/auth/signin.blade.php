...
<body class="bg-login">
    ...
    <script>
        ...
        // This must be inside document.ready and AFTER Parsley.js is loaded
        $('#formSubmit').parsley(); // Initialize Parsley once the DOM is ready
        ...
        $("#formSubmit").submit(function(e){
            ...
            if($(this).parsley().validate()){
                var url = "{{ url('login_user') }}"; // Laravel helper for URL
                $.ajax({
                    url: url,
                    ...
                    success:function(result){
                        if(result.status == 200){
                            ...
                            // Consider using a more user-friendly notification library like Toastr or SweetAlert
                            alert('Wrong credentials');
                        }
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                        // Handle AJAX errors (e.g., server errors, network issues)
                        console.error("AJAX Error:", textStatus, errorThrown, jqXHR.responseText);
                        alert("An error occurred during login. Please try again.");
                    }
                });
            } else {
                // Parsley will display validation errors automatically.
                // This alert might be redundant if Parsley's UI is good.
                // alert('Please correct the form errors.');
                console.log("Form validation failed by Parsley.");
            }
        });
